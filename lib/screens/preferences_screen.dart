...
// ════════════════════════════════════════════════════
// شاشة أرصدة البداية — تشمل الصندوق + رأس المال + الزبائن + الموردين
// ════════════════════════════════════════════════════
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _CustomerBalanceCard(
          customer: customer,
          onSave: (balance, startDate) async {
            await _customerService.setInitialBalance(customer.name, balance);
            if (startDate.isNotEmpty) {
              await _customerService.updateCustomerStartDate(
                  customer.name, startDate);
            }
            await _loadAll();
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text('✅ تم حفظ بيانات ${customer.name}'),
                    backgroundColor: Colors.green),
              );
            }
          },
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _SupplierBalanceCard(
          supplier: supplier,
          onSave: (balance, startDate) async {
            await _supplierService.setInitialBalance(supplier.name, balance);
            if (startDate.isNotEmpty) {
              await _supplierService.updateSupplierStartDate(
                  supplier.name, startDate);
            }
            await _loadAll();
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text('✅ تم حفظ بيانات ${supplier.name}'),
                    backgroundColor: Colors.green),
              );
            }
          },
